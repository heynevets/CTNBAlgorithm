...
def my_replace(df, myDict):
    returnDF = df['Text'].replace(myDict, regex = True, inplace = True)
    return returnDF, 
data['new_number'] = data.progress_apply(my_func, axis = 1)

dfq['Text'].replace(myDict, regex = True, inplace = True) # I dont know if this is faster
...
dfq.to_csv('QuestionCleaned.csv')
# Use LDA on Topics
